...
CASES = ("healthy", "lost-response", "kill-after-persistence", "stale-between-get-patch",
         "intervening-writer", "recreated-same-revision", "pending-no-watch")


def deployment():
    return {
        "apiVersion": "apps/v1", "kind": "Deployment",
        "metadata": {"name": "api", "namespace": "demo", "uid": "original-uid",
                     "resourceVersion": "10", "generation": 1,
...
class Fixture:
    ...
    def writer(self, recreate=False, same_revision=False):
        with self.lock:
            ...
            self.effects.append({"actor": "fixture-writer", "before": before,
                                 "after": facts(self.obj)})
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                if path == "/api":
                    ...
                    self.reply({"kind": "APIResourceList", "apiVersion": "v1",
                                "groupVersion": "apps/v1" if "apps" in path else "v1",
                                "resources": [{"name": "deployments", "singularName": "deployment",
                                               "namespaced": True, "kind": "Deployment",
                                               "verbs": ["get", "list", "watch", "patch"]}]
                                if "apps" in path else []})
                    ...
                    if parse_qs(urlsplit(self.path).query).get("watch") == ["true"]:
                        ...
                        self.reply({"apiVersion": "apps/v1", "kind": "DeploymentList",
                                    "metadata": {"resourceVersion": current["metadata"]["resourceVersion"]},
                                    "items": [current]})
                else:
                    fixture.errors.append("unexpected GET " + self.path)
                    self.reply({"kind": "Status", "status": "Failure", "message": "unexpected path"}, 404)
            ...
            def do_PATCH(self):
                ...
                if (urlsplit(self.path).path != RESOURCE + "/api" or patch != expected
                ...
                with fixture.lock:
                    ...
                    fixture.obj["metadata"]["annotations"]["deployment.kubernetes.io/revision"] = "2"
                    fixture.obj["status"]["availableReplicas"] = 0
                    current = copy.deepcopy(fixture.obj)
                    fixture.effects.append({"actor": "kubectl-patch", "before": before,
                                            "after": facts(current)})
    ...
    def settle(self):
        with self.lock:
            self.obj["status"].update(observedGeneration=self.obj["metadata"]["generation"],
                                      availableReplicas=1)
...
def environment(kubectl, fault=None):
    ...
    with tempfile.TemporaryDirectory(prefix="kubectl-corpus-") as directory:
        ...
        config.write_text(json.dumps({
            ...
            "contexts": [{"name": "fixture", "context": {"cluster": "fixture", "namespace": "demo"}}],
            "users": [],
        }))
        env = {"PATH": os.defpath, "HOME": directory, "TMPDIR": directory,
               "KUBECONFIG": str(config), "LANG": "C", "LC_ALL": "C"}
        base = [kubectl, "--kubeconfig", str(config), "--context=fixture", "--server", endpoint,
                "--namespace=demo", "--cache-dir", str(root / "cache"), "--request-timeout=3s"]

        def run(args, kill=False):
            started = time.monotonic()
            process = subprocess.Popen(base + args, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                       text=True)
            try:
                if kill:
            ...
            record = {"argv": args, "exit": process.returncode, "stdout": out, "stderr": err,
                      "killed": kill, "elapsed_s": round(time.monotonic() - started, 3)}
...
def experiment(kubectl, case):
    fault = case if case in ("lost-response", "kill-after-persistence", "stale-between-get-patch") else None
    with environment(kubectl, fault) as (fixture, run):
        intended = {**facts(fixture.obj), "image": IMAGE}
        initial = run(["set", "image", "deployment/api", "api=" + IMAGE],
        ...
        if case in ("intervening-writer", "recreated-same-revision"):
            fixture.writer(recreate=case == "recreated-same-revision",
        ...
        return {"case": case, "intended": intended, "caller": fixture.calls,
                "receiver_requests": fixture.requests, "fixture_effects": fixture.effects,
                "retained_state": {"caller": "argv and captured output only, no action journal",
...
def main():
    ...
    version = json.loads(subprocess.check_output([kubectl, "version", "--client", "-o", "json"], timeout=5))
    ...
    print(json.dumps({"client": client, "cases": cases,
                      "elapsed_s": round(time.monotonic() - started, 3)}, indent=2))

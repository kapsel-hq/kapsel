...
mod tests {
    ...
    proptest! {
        ...
        #[test]
        // #[ignore] // Disabled: The complex, stateful setup/teardown lifecycle of the
        // production DeliveryEngine within TestEnv is incompatible with the rapid,
        // stateless, and randomized action sequences generated by proptest.
        // Reactivating these tests will require a significant redesign of either
        // the property test state machine or the engine's test harness integration.
        #[ignore = "DeliveryEngine lifecycle incompatible with rapid proptest operations"]
        fn system_invariants_under_random_operations(
            actions in prop::collection::vec(action_strategy(), 1..20)
        ) {
            let rt = tokio::runtime::Runtime::new().unwrap();
            rt.block_on(async {
                run_stateful_property_test("random_operations", actions)
                    .await
                    .unwrap();
            });
        }
        ...
        fn system_handles_failure_scenarios(
            ...
            let rt = tokio::runtime::Runtime::new().unwrap();
            rt.block_on(async {
                run_stateful_property_test("failure_scenarios", actions)
                    .await
                    .unwrap();
            });
        }

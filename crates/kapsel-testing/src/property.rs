...
impl PropertyTestState {
    ...
    async fn execute_delivery_cycle(&mut self) -> Result<()> {
        ...
        // Note: run_delivery_cycle doesn't return a count, so we can't track it here
        self.total_actions += 1;
        Ok(())
    }
    ...
    async fn check_state_machine_integrity(&self) -> Result<()> {
        ...
        for event in events {
            ...
            match event.status.as_str() {
                "received" | "pending" | "delivering" | "delivered" | "failed" | "dead_letter" => {
                    // Valid states
                },
                invalid => {
            ...
            if event.status == "delivered" && event.failure_count > 0 {

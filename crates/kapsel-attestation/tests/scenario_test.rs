...
async fn multiple_deliveries_create_multiple_attestation_leaves() -> anyhow::Result<()> {
    TestEnv::run_isolated_test(|mut env| async move {
        ...
        let tenant_id = env.create_tenant("Test Tenant Multi").await?;
        let endpoint_id = env
        ...
        // Verify all attestation leaves were created
        {
            ...
            assert_eq!(
                pending_count, 3,
                "Each successful delivery should create one attestation leaf"
            );
        }
...
async fn failed_delivery_does_not_create_attestation_leaf() -> anyhow::Result<()> {
    TestEnv::run_isolated_test(|mut env| async move {
        ...
        // Create test setup - no transaction needed for isolated tests
        let tenant_id = env.create_tenant("Test Tenant Failure").await?;
        let endpoint_id = env
            .create_endpoint_with_config(
                tenant_id,
                &mock_server.uri(),
...
async fn multiple_events_enable_batch_attestation_commitment() -> anyhow::Result<()> {
    TestEnv::run_isolated_test(|env| async move {
        ...
        let initial_tree_size = env.storage().signed_tree_heads.find_max_tree_size().await?;
        ...
        {
            let signed_tree_head = merkle_service.write().await.try_commit_pending().await?;

            let expected_tree_size = u64::try_from(initial_tree_size)? + event_count as u64;
            assert_eq!(
